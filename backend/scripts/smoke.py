...
def main():
    ...
    h_stu1 = login("student1")
    h_stu2 = login("student2")
    h_stu3 = login("student3")
    h_lect = login("lecturer1")
    h_advisor = login("advisor1")
    h_office = login("ptdt")
    check("đăng nhập đủ 6 tài khoản demo", True)

    # Lấy id lớp đang mở kỳ 2026-T1
    classes = httpx.get(f"{BASE}/course-classes", params={"status": "open"}, headers=h_stu1).json()
    by_course = {c["course_code"]: c for c in classes}
    ctdl = by_course["CTDL"]
    csdl = by_course["CSDL"]
    oop = by_course["OOP"]

    # 1) student1 (đã đạt TH1) đăng ký CTDL → 201
    r = httpx.post(f"{BASE}/enrollments", json={"course_class_id": ctdl["id"]}, headers=h_stu1)
    check("student1 đăng ký CTDL.A → 201", r.status_code == 201, r.text[:120])
    enr_stu1_ctdl = r.json().get("id")

    # 2) student2 (trượt TH1) đăng ký CTDL → 400 tiên quyết
    r = httpx.post(f"{BASE}/enrollments", json={"course_class_id": ctdl["id"]}, headers=h_stu2)
    check("student2 bị chặn tiên quyết CTDL.A → 400", r.status_code == 400, r.text[:120])

    # 3) CSDL.A đã đủ chỗ từ seed (SV003+SV004, max_size=2) → mọi đăng ký mới đều 400
    r = httpx.post(f"{BASE}/enrollments", json={"course_class_id": csdl["id"]}, headers=h_stu1)
    check("student1 đăng ký CSDL.A đầy lớp → 400", r.status_code == 400, r.text[:120])
    r = httpx.post(f"{BASE}/enrollments", json={"course_class_id": csdl["id"]}, headers=h_stu3)
    check("student3 đăng ký CSDL.A đầy lớp → 400", r.status_code == 400, r.text[:120])

    # 4) student1 đăng ký OOP.A trùng lịch CTDL.A → 400; hơn nữa chưa đạt CTDL
    r = httpx.post(f"{BASE}/enrollments", json={"course_class_id": oop["id"]}, headers=h_stu1)
    check("student1 đăng ký OOP.A (chưa đạt CTDL + trùng lịch) → 400", r.status_code == 400, r.text[:120])
    ...
    check("student1 hủy đăng ký CTDL.A → 200", r.status_code == 200, r.text[:120])
    r = httpx.post(f"{BASE}/enrollments", json={"course_class_id": ctdl["id"]}, headers=h_stu1)
    check("student1 đăng ký lại CTDL.A → 201", r.status_code == 201, r.text[:120])

    # 7) ranh giới nhập điểm: lấy 1 enrollment kỳ 2025 (TH1.A của SV003)
    enr_stu3 = httpx.get(f"{BASE}/enrollments/student/3", headers=h_advisor).json()
    ...
    r = httpx.get(f"{BASE}/grades/student/1", headers=h_stu2)
    check("student xem điểm sinh viên khác → 403", r.status_code == 403, r.text[:80])
    r = httpx.get(f"{BASE}/grades/student/1", headers=h_stu1)
    check("student xem bảng điểm của mình → 200", r.status_code == 200)

    # 8b) thời khóa biểu sinh viên (student1 đã đăng ký CTDL kỳ 2026-T1)
    r = httpx.get(f"{BASE}/schedule/student/1", headers=h_stu1)
    ...
    r = httpx.get(f"{BASE}/schedule/student/1", headers=h_stu2)
    check("student xem thời khóa biểu sinh viên khác → 403", r.status_code == 403)
    r = httpx.get(f"{BASE}/schedule/student/1", headers=h_lect)
    check("lecturer xem thời khóa biểu sinh viên → 403", r.status_code == 403)
    r = httpx.get(f"{BASE}/schedule/student/1", params={"year": 2026, "term": 1}, headers=h_office)
    ...
    r = httpx.post(f"{BASE}/ai/course-advice", json={"student_id": 1}, headers=h_stu1)
    body = r.json()
    check(
        "AI course-advice student1 → 200, có eligible",
        r.status_code == 200 and len(body.get("eligible_classes", [])) >= 1,
        r.text[:150],
    )
    r = httpx.post(f"{BASE}/ai/course-advice", json={"student_id": 1}, headers=h_stu2)
    check("AI course-advice hộ người khác → 403", r.status_code == 403)
    r = httpx.post(f"{BASE}/ai/study-summary", json={"student_id": 1}, headers=h_advisor)
